package com.bgsoftware.superiorskyblock.plot.bank;
...
import com.bgsoftware.superiorskyblock.plot.bank.logs.CacheBankLogs;
import com.bgsoftware.superiorskyblock.plot.bank.logs.DatabaseBankLogs;
import com.bgsoftware.superiorskyblock.plot.bank.logs.IBankLogs;
...
public class SPlotBank implements PlotBank {
    ...
    public SPlotBank(Plot plot, Supplier<Boolean> isGiveInterestFailed) {
        this.plot = plot;
        this.isGiveInterestFailed = isGiveInterestFailed;
        this.bankLogs = BuiltinModules.BANK.cacheAllLogs ? new CacheBankLogs() : new DatabaseBankLogs(plot);
    }
    ...
    public BankTransaction depositMoney(SuperiorPlayer superiorPlayer, BigDecimal amount) {
        ...
        Log.debug(Debug.DEPOSIT_MONEY, plot.getOwner().getName(), superiorPlayer.getName(), amount);

        BankTransaction bankTransaction;
        String failureReason;

        if (!plot.hasPermission(superiorPlayer, PlotPrivileges.DEPOSIT_MONEY)) {
            ...
            EventResult<String> eventResult = plugin.getEventsBus().callPlotBankDepositEvent(superiorPlayer, plot, amount);
        ...
        if (Text.isBlank(failureReason)) {
            ...
            PlotUtils.sendMessage(plot, Message.DEPOSIT_ANNOUNCEMENT, Collections.emptyList(), superiorPlayer.getName(),
                    Formatters.NUMBER_FORMATTER.format(amount));

            plugin.getMenus().refreshBankLogs(plot);
            plugin.getMenus().refreshBankLogs(plot);
        } else {
    ...
    public BankTransaction depositAdminMoney(CommandSender commandSender, BigDecimal amount) {
        ...
        Log.debug(Debug.DEPOSIT_MONEY, plot.getOwner().getName(), commandSender.getName(), amount);

        EventResult<String> eventResult = plugin.getEventsBus().callPlotBankDepositEvent(commandSender instanceof Player ?
                plugin.getPlayers().getSuperiorPlayer(commandSender) : null, plot, amount);
    ...
    public boolean canDepositMoney(BigDecimal amount) {
        Preconditions.checkNotNull(amount, "amount parameter cannot be null.");
        return this.plot.getBankLimit().compareTo(NO_BANK_LIMIT) <= 0 ||
                this.balance.get().add(amount).compareTo(this.plot.getBankLimit()) <= 0;
    }
    ...
    public BankTransaction withdrawMoney(SuperiorPlayer superiorPlayer, BigDecimal amount, @Nullable List<String> commandsToExecute) {
        ...
        Log.debug(Debug.WITHDRAW_MONEY, plot.getOwner().getName(), superiorPlayer.getName(), amount, commandsToExecute);
        ...
        if (!plot.hasPermission(superiorPlayer, PlotPrivileges.WITHDRAW_MONEY)) {
            failureReason = "No permission";
            ...
            EventResult<String> eventResult = plugin.getEventsBus().callPlotBankWithdrawEvent(superiorPlayer, plot, withdrawAmount);
        ...
        if (Text.isBlank(failureReason)) {
            ...
            PlotUtils.sendMessage(plot, Message.WITHDRAW_ANNOUNCEMENT, Collections.emptyList(), superiorPlayer.getName(),
    ...
    public BankTransaction withdrawAdminMoney(CommandSender commandSender, BigDecimal amount) {
        ...
        Log.debug(Debug.WITHDRAW_MONEY, plot.getOwner().getName(), commandSender.getName(), amount);

        UUID senderUUID = commandSender instanceof Player ? ((Player) commandSender).getUniqueId() : null;

        int position = this.bankLogs.getLastTransactionPosition() + 1;

        EventResult<String> eventResult = plugin.getEventsBus().callPlotBankWithdrawEvent(commandSender instanceof Player ?
                plugin.getPlayers().getSuperiorPlayer(commandSender) : null, plot, amount);
    ...
    private void addTransaction(BankTransaction bankTransaction, boolean save) {
        ...
        if (save) {
            PlotsDatabaseBridge.saveBankTransaction(plot, bankTransaction);
        }
    }
    ...
    private void increaseBalance(BigDecimal amount) {
        ...
        PlotsDatabaseBridge.saveBankBalance(plot);
    }
