package com.bgsoftware.superiorskyblock.plot.role;
...
import com.bgsoftware.superiorskyblock.plot.role.container.RolesContainer;
...
public class RolesManagerImpl extends Manager implements RolesManager {
    ...
    public void loadData() throws ManagerLoadException {
        ConfigurationSection rolesSection = plugin.getSettings().getPlotRoles().getSection();

        ConfigurationSection guestSection = rolesSection.getConfigurationSection("guest");

        if (guestSection == null)
            throw new ManagerLoadException("Missing \"guest\" section for plot roles", ManagerLoadException.ErrorLevel.SERVER_SHUTDOWN);

        ConfigurationSection coopSection = rolesSection.getConfigurationSection("coop");

        if (coopSection == null)
            throw new ManagerLoadException("Missing \"coop\" section for plot roles", ManagerLoadException.ErrorLevel.SERVER_SHUTDOWN);
        ...
        if (laddersSection == null)
            throw new ManagerLoadException("Missing \"ladder\" section for plot roles", ManagerLoadException.ErrorLevel.SERVER_SHUTDOWN);
