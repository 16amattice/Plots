...
public class DefaultPlotBlocksTrackerAlgorithm implements PlotBlocksTrackerAlgorithm {
    ...
    public DefaultPlotBlocksTrackerAlgorithm(Plot plot) {
        this.plot = plot;
    }
    ...
    public boolean trackBlock(Key key, BigInteger amount) {
        ...
        boolean hasBlockLimit = plot.getBlockLimit(key) != -1;
        boolean valuesMenu = plugin.getBlockValues().isValuesMenu(key);

        if (increaseAmount || hasBlockLimit || valuesMenu) {
            Log.debug(Debug.BLOCK_PLACE, plot.getOwner().getName(), key, amount);
    ...
    public boolean untrackBlock(Key key, BigInteger amount) {
        ...
        if (decreaseAmount || hasBlockLimit || valuesMenu) {
            Log.debug(Debug.BLOCK_BREAK, plot.getOwner().getName(), key, amount);

            Key valueKey = plugin.getBlockValues().getBlockKey(key);
            removeCounts(valueKey, amount);

            Key limitKey = plot.getBlockLimitKey(valueKey);
    ...
    private void addCounts(Key key, BigInteger amount) {
        ...
        Log.debug(Debug.BLOCK_COUNT_INCREASE, plot.getOwner().getName(), key, amount);
    ...
    private void removeCounts(Key key, BigInteger amount) {
        Log.debug(Debug.BLOCK_COUNT_DECREASE, plot.getOwner().getName(), key, amount);
