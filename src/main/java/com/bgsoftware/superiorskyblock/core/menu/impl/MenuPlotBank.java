...
public class MenuPlotBank extends AbstractMenu<PlotMenuView, PlotViewArgs> {

    private MenuPlotBank(MenuParseResult<PlotMenuView> parseResult) {
        super(MenuIdentifiers.MENU_PLOT_BANK, parseResult);
    }
    ...
    public void handleDeposit(SuperiorPlayer superiorPlayer, Plot plot, BankTransaction bankTransaction,
        ...
        if (bankTransaction.getFailureReason().isEmpty()) {
            ...
            if (!failureReason.isEmpty()) {
                switch (failureReason) {
                    case "No permission":
                        Message.NO_DEPOSIT_PERMISSION.send(superiorPlayer, plot.getRequiredPlayerRole(PlotPrivileges.DEPOSIT_MONEY));
                        break;
                    case "Invalid amount":
    ...
    public void handleWithdraw(SuperiorPlayer superiorPlayer, Plot plot, BankTransaction bankTransaction,
        ...
        if (bankTransaction.getFailureReason().isEmpty()) {
            ...
            if (!failureReason.isEmpty()) {
                switch (failureReason) {
                    case "No permission":
                        Message.NO_WITHDRAW_PERMISSION.send(superiorPlayer, plot.getRequiredPlayerRole(PlotPrivileges.WITHDRAW_MONEY));
                        break;
                    case "Invalid amount":
                        Message.INVALID_AMOUNT.send(superiorPlayer, Formatters.NUMBER_FORMATTER.format(amount));
                        break;
                    case "Bank is empty":
                        Message.PLOT_BANK_EMPTY.send(superiorPlayer);
                        break;
                    default:
    ...
    @Nullable
    public static MenuPlotBank createInstance() {
        MenuParseResult<PlotMenuView> menuParseResult = MenuParserImpl.getInstance().loadMenu("plot-bank.yml",
                null);
        ...
        return new MenuPlotBank(menuParseResult);
    }
